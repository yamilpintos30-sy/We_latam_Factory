...
const plan = await fetch("https://api.elevenlabs.io/v1/music/plan", { method: "POST", headers, body: JSON.stringify({ prompt, music_length_ms: 4000, model_id: "music_v2" }) });
console.log(`plan=${plan.status}`);
if (!plan.ok) process.exitCode = 1;
else {
  const compositionPlan = await plan.json();
  const detailed = await fetch("https://api.elevenlabs.io/v1/music/detailed?output_format=pcm_48000", { method: "POST", headers, body: JSON.stringify({ composition_plan: compositionPlan, model_id: "music_v2", with_timestamps: true, with_waveform_visual: true }) });
